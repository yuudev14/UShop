...
const checkout = async (req, res) => {
  try {
    ...
    const buyItems_product_id = buyItems
    ...
    if (
      updatedBuyItems.every((prod) => Number(prod.stock) >= Number(prod.items))
    ) {
      ...
      updatedBuyItems.forEach(async (prod, i) => {
        try {
          ...
          if (error.constraint === "check_stock") {
            const product = buyItems.filter((prod) =>
              error.detail.includes(prod.product_id)
            )[0];
            res
              .status(400)
              .send(`${product.product_name} items exceeds product's stocks`);
          }
        }
      });
    } else {
