...
const addProducts = async (req, res) => {
  let { productName, category, price, newImages, description, stock } =
    req.body;
  try {
    const product = await db.query(
      ...
      [req.user, productName, Number(price), description, stock]
    );
...
const filterProducts = async (req, res) => {
  try {
    const { productName, category, minStock, maxStock, minPrice, maxPrice } =
      req.body;
...
const modifyProduct = async (req, res) => {
  try {
    const product_id = req.params.product_id;
    const { productName, category, price, newImages, description, stock } =
      req.body;
    await db.query(
      `UPDATE products
            SET product_name = $1,
            ...
            WHERE product_id = $5
            AND shop_id = $6`,
      [productName, price, description, stock, product_id, req.user]
    );
