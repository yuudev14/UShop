...
const MainNav2 = () => {
    ...
    useEffect(() => {
        if(searchInput !== ''){
            (async() => {
                try {
                    const search = await axios.post('/ushop/search', {search: searchInput})
                    ...
                    {searchResults.products.length ? (
                        <div className='productsSearch'>
                            ...
                            {searchResults.products.map(prod => (
                                <Link to={`/product/${prod.product_id}`} onClick={() => setSearchInput('')}>
                                    <img src={prod.images} />
                                    <div className='info'>
                    ...
                    {searchResults.shops.length ? (
                        <div className='shopsSearch'>
                            ...
                            {searchResults.shops.map(shop => (
                                <Link to={`/shop/${shop.shop_name}`} onClick={() => setSearchInput('')}>
                                    <img src={shop.logo} />
                                    <div className='info'>
