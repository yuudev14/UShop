...
const Shop = () => {
  ...
  useEffect(() => {
    shopAndImage.current.style = `
        ...
        `;
    (async () => {
      const shopDetails = await axios.get(`/ushop/get-shop-info/${shop_name}`, {
  ...
  const follow_unfollow = async () => {
    try {
      const follow = await axios.post(
        `/ushop/follow/${shopDetails.shop_id}`,
        {},
        { headers: { token: JSON.parse(localStorage.getItem("UShop")).token } }
      );
      setShopDetails({
      ...
      <div className="product_shop">
        <div className="shopNameAndImg">
          ...
          {auth.isAuth === true && (
            <button onClick={follow_unfollow}>
              {!shopDetails.follows ? "Follow" : "Unfollow"}
            </button>
          )}
        </div>
        <div className="shopInfoSummary">
          ...
          <div className="info">
            <p>joined</p>
            <p className="value">
              {new Date(shopDetails.date).toLocaleDateString()}
            </p>
          </div>
        </div>
      </div>
