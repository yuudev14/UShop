    ...
    try {
      ...
      manageProductsForm.images.forEach(async (img, i) => {
        ...
        if (i === manageProductsForm.images.length - 1) {
          const beforeImg = manageProductsForm.images
            .filter((img) => !img.image_link.includes("data:image"))
            .map((img) => img.image_link);
          await axios.post(
            `/sell-ushop/modify-product/${product_id}`,
            { ...manageProductsForm, newImages: [...beforeImg, ...newImages] },
            {
              headers: {
      ...
      if (e.target.files.length !== 0) {
        ...
        reader.onload = () => {
          setManageProductsForm({
            ...
            sampleImages: [
              ...manageProductsForm.sampleImages,
