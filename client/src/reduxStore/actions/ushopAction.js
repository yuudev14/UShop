  ...
  return async (dispatch) => {
    try {
      const products = await axios.post(
        `/ushop/get-${filter}-shops-product/0`,
        { shop_name }
      );
      dispatch({
        type: SET_USHOP_PRODUCT,
...
export const getUsersFollowProductsAction = (filter) => {
  return async (dispatch) => {
    try {
      const products = await axios.get(`/profile/${filter}-follow-products/0`, {
...
export const getCategoryProductsAction = (filter, category) => {
  return async (dispatch) => {
    try {
      const products = await axios.post(
        `/ushop/get-${filter}-category-product/0`,
...
export const seeMoreCategoryProductsAction = (filter, category, start) => {
  return async (dispatch) => {
    try {
      const products = await axios.post(
        `/ushop/get-${filter}-category-product/${start}`,
        { category },
        { headers: { token: JSON.parse(localStorage.getItem("UShop")).token } }
      );
      dispatch({
  ...
  return async (dispatch) => {
    try {
      const products = await axios.get(
        `/profile/${filter}-follow-products/${start}`,
        { headers: { token: JSON.parse(localStorage.getItem("UShop")).token } }
      );
      dispatch({
        type: SEE_MORE_USHOP_PRODUCT,
...
export const seeMoreShopProductsAction = (filter, shop_name, start) => {
  return async (dispatch) => {
    try {
      const products = await axios.post(
        `/ushop/get-${filter}-shops-product/${start}`,
        { shop_name }
      );
      dispatch({
        type: SEE_MORE_USHOP_PRODUCT,
...
export const seeMoreUshopProductListAction = (start, filter) => {
  return async (dispatch) => {
    try {
      const products = await axios.get(
        `/ushop/get-${filter}-products/${start}`
      );
      dispatch({
        type: SEE_MORE_USHOP_PRODUCT,
        data: products.data,
...
export const getAllOrdersAction = () => {
  return async (dispatch) => {
    try {
      const orders = await axios.get("/profile/all-orders", {
        headers: { token: JSON.parse(localStorage.getItem("UShop")).token },
      });
      dispatch({
...
export const getToShipOrdersAction = () => {
  return async (dispatch) => {
    try {
      const orders = await axios.get("/profile/to-ship-orders", {
        headers: { token: JSON.parse(localStorage.getItem("UShop")).token },
      });
      dispatch({
        type: SET_BUYER_ORDERS,
