...
import { CHECK_ALL_CART, CHECK_CART, RESET_CART, SET_CART, UPDATE_ITEM_NO_CART } from "./types";
...
export const getCartProductAction = () => {
    return async(dispatch) => {
        try {
            ...
            if(ushop){
                const cartProduct = await axios.get('/cart/get-cart', {headers : {token : JSON.parse(localStorage.getItem('UShop')).token}});
...
export const addCartAction = (product_id) => {
    return async(dispatch) => {
        try {
            const product = await axios.post('/cart/add-cart', {product_id}, {headers : {token : JSON.parse(localStorage.getItem('UShop')).token}});
            const prod = product.data.map(prod => {
                prod.totalPrice = prod.price
                prod.checked = false
                return prod
...
export const checkOutAction = (data) => {
    return async(dispatch) => {
        try {
            const checkout = await axios.post('/ushop/checkout', {buyItems : data}, {headers : {token : JSON.parse(localStorage.getItem('UShop')).token}});
            if(checkout.data){
                const cartProduct = await axios.get('/cart/get-cart', {headers : {token : JSON.parse(localStorage.getItem('UShop')).token}});
                const prod = cartProduct.data.map(prod => {
                                prod.totalPrice = prod.price
